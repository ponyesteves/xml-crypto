...
ExclusiveCanonicalization.prototype.renderNs = function(node, prefixesInScope, defaultNs, defaultNsForPrefix, inclusiveNamespacesPrefixList, options) {
  ...
  if (node.prefix) {
    if (prefixesInScope.indexOf(node.prefix)==-1) {
      ...
      if(options && options.aditionalNameSpaces){
        for(let adtionalNameSpaces of options.aditionalNameSpaces){
          let {prefix, namespaceURI} = adtionalNameSpaces
      ...
      // nsListToRender.push({"prefix": 'xd', "namespaceURI": node.namespaceURI });
      ...
      if(defaultNs) {
        res.push(' ' + defaultNs)
      } // @ponyesteves saco temporalmente esto
  ...
  if (node.attributes) {
    for (i = 0; i < node.attributes.length; ++i) {
      ...
      // Agregado por @ponyesteves. Es para el caso del campo Referencia que tenia xmlns definido a pesar de ya tener un prefijo ns0.
      // Lo testeo a Referencia contra Regexp porque nodeName puede venir con prefijo o sin a veces es
      // ns:Referencia otras solo Referencia de esta manera contemplo las dos opciones
      if (/Referencia/.test(node.nodeName) && options && options.isC14n && attr.localName === 'xmlns'){
