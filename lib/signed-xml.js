...
SignedXml.CanonicalizationAlgorithms = {
  ...
  'http://www.w3.org/TR/2001/REC-xml-c14n-20010315': ExclusiveCanonicalization // @ponyesteves
}
...
SignedXml.prototype.checkSignature = function(xml) {
  ...
  if (!this.validateSignatureValue(xml)) {
    return false;
  }
...
SignedXml.prototype.validateSignatureValue = function(xml) {
  console.log('VALIDANDO FIRMA')
  /**
   * @ponyesteves
   */
  xml = new Dom().parseFromString(xml)
  let firstChild = xml.firstChild
  let prefix
  let nameSpaceUri = firstChild.namespaceURI

  var options = this.options // @ponyesteves
  options.aditionalNameSpaces = []
 
  if(firstChild.prefix && firstChild.namespaceURI){
    prefix = `xmlns:${firstChild.prefix}="${firstChild.namespaceURI}"`
  } else {
    for(let inclusiveNamespacesPrefix of this.options.inclusiveNamespacesPrefixList){
      if(aditionaNameSpace = firstChild._nsMap[inclusiveNamespacesPrefix]){
        options.aditionalNameSpaces.push({prefix: inclusiveNamespacesPrefix, namespaceURI: aditionaNameSpace})
      }
    }
  }
  ...
  */
  ...
  // Si el documento tiene prefijo @ponyesteves agrega a la firma el namespace
  console.log('ANTES')
  if(this.canonicalizationAlgorithm == 'http://www.w3.org/TR/2001/REC-xml-c14n-20010315'){
    if(prefix){
            console.log('primero')
      options.defaultNs = prefix
      // options = {defaultNs: 'xmlns:ns0="http://cfe.dgi.gub.uy"'  } 
      // options = {defaultNs: `xmlns:cfe="http://cfe.dgi.gub.uy"`}
    } else if (!Object.keys(signedInfo[0].parentNode._nsMap).includes('')) {
      console.log('SEGUNDO')
      console.log(Object.keys(signedInfo[0].parentNode._nsMap))
      /**
       * Si el documento CFE no tiene prefijo y la firma tiene prefijo ds:Signature
       * y xlmns:ds="www...." para ese prefijo debo agregar pero no tiene xmlns como attributo
       * se lo debo agregar. signedInfo[0].parentNode._nsMap devuelve un objeto con los ns.. tiene que tener un con key '' que es xmlns vacio
       */
      options.defaultNs = 'xmlns="http://cfe.dgi.gub.uy"'
    }
  }

  var signedInfoCanon = this.getCanonXml([this.canonicalizationAlgorithm], signedInfo[0], options)
  console.log(signedInfoCanon.toString())
...
SignedXml.prototype.validateReferences = function(doc) {
  for (var r in this.references) {
    ...
    ref.inclusiveNamespacesPrefixList = ref.inclusiveNamespacesPrefixList || []
    this.options.inclusiveNamespacesPrefixList = this.options.inclusiveNamespacesPrefixList || []
    let inclusiveNamespacesPrefixList = ref.inclusiveNamespacesPrefixList.concat(this.options.inclusiveNamespacesPrefixList)
    var canonXml = this.getCanonXml(ref.transforms, elem[0], { inclusiveNamespacesPrefixList });
